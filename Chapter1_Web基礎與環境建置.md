...
- **IP 地址 Internet Protocol Address**
  ...
  **1. 格式演進與危機：IPv4 與 IPv6 比較**
  ...
  > 📝 **小提醒**
  > IPv6 中的 `::` 代表「一連串連續的 0 省略不寫」。所以本機位址 `0000:...:0001` 才能縮寫成 `::1`（一個位址中只能使用一次 `::`）。
  ...
  了解 IP 的長相以及 IPv4 數量枯竭的問題後，網路工程師發明了「私有 IP」來當作解方。

  **2. 解決枯竭的策略：公開 IP 與私有 IP 比較**
  ...
  > 📝 **私有網段不用死背，開發久了自然會記得**
  > 以上三個區段是國際網路標準特別「保留」作為私有網路用途的 IP 範圍（其中 172 網段的第二碼固定為 16 ~ 31，此為標準規範，並非講義誤植）。
  > 但學習時請把握一個核心觀念：只要看到這三組特徵的 IP 地址，就代表該設備目前正身處於「區域網路」環境中。

  **3. 讓私有 IP 連上網：NAT 機制 (Network Address Translation)**

  為了解決 IPv4 枯竭的燃眉之急而誕生的技術。它可以讓區域網路內的多台設備，各自擁有私有 IP，**共用同一個對外的「公開 IP」** 來連上網際網路。這是目前最普遍的做法！

  以現實生活來舉例的話：一家公司雖然有數十位員工，但對外辦公室只有一組代表電話號碼。外部來電打這個號碼，總機再轉接給內部分機。NAT 就像這個公司的「總機系統」，Router 路由器讓所有家中的設備各自擁有私有 IP 分機，卻共用同一個公開 IP 代表號碼與外界通訊。
...
**4. 區域網段的劃分：子網路遮罩 Subnet Mask**

用來區分 IP 地址中的「網路段」與「主機段」，幫助設備判斷目標 IP 是否與自己處在同一個區域網路內。

> 💡 **白話文解釋：什麼是 /24 或是 255.255.255.0？**
>
> 我們常看到 IP 位址後面跟著 `/24` 或是子網路遮罩 `255.255.255.0`。簡單來說，這就是在告訴電腦：**「前面三個數字是固定的社區名稱，最後一個數字是可分配的住戶門牌。」**
>
> **為什麼 `/24` 和 `255.255.255.0` 是同一件事？**
> 還記得前面說過，IP 的每一組數字都是 8 位元嗎？`/24` 的意思就是「從最前面算起，**有 24 個位元屬於社區名稱（網路段）**」。而 24 ÷ 8 = 3，剛好就是前三組數字；換算成遮罩，被固定的三組就寫成 `255.255.255`，可自由分配的最後一組則是 `0`，合起來正是 `255.255.255.0`。
>
> 以 `192.168.1.100/24` 為例：
>
> - **社區名稱 (網路段)**：`192.168.1`。只要前三個數字一樣，就代表大家住在同一個區網社區裡。
> - **住戶門牌 (主機段)**：最後一個數字。雖然 `0 ~ 255` 看似有 256 個，但頭尾兩個有特殊用途——`.0` 代表「社區本身」（網路位址）、`.255` 代表「對全社區廣播」（廣播位址），這兩個不能分給設備。所以實際可用的是 `1 ~ 254`，最多 254 台。

<br>

_透過上述的 IP 地址與 NAT 機制，資料已經成功送達了正確的電腦（伺服器主機）。但一台電腦可能同時執行著網頁伺服器與資料庫，我們要怎麼保證資料交給正確的軟體？這時就需要 Port 埠號。_
...
|   協定名稱    | 英文全名                           |      中文全名      | 說明                                                                                           |
| :-----------: | :--------------------------------- | :----------------: | :--------------------------------------------------------------------------------------------- |
|    **IP**     | Internet Protocol                  |    網際網路協定    | 網路通訊的基石，負責尋找目標裝置的位址與傳遞路徑（如同物流系統中的尋址）。                     |
|    **TCP**    | Transmission Control Protocol      |    傳輸控制協定    | 建立於 IP 之上，負責將資料「準確無誤」地從 A 點傳送到 B 點，確保不掉包（如同掛號信需簽收）。   |
|    **UDP**    | User Datagram Protocol             |  使用者資料包協定  | 建立於 IP 之上，不保證資料一定送達，但「傳輸速度極快」（如同平信）。常用於影音串流或連線遊戲。 |
...
|  4  | 網路通訊協定與 HTTP 基礎   | HTTP 的三大核心特性為何？                        | ① 未加密明文傳輸。<br>② 基於請求與回應模型。 Request-Response<br>③ 無狀態 Stateless。                                                                                                                                                                                      |
|  5  | 網路通訊協定與 HTTP 基礎   | GET 與 POST 方法有何不同？                       | GET 用於取得資源，參數附加於 URL Query String。<br>POST 用於提交資料，參數封裝在 Request Body 內。                                                                                                                                                                         |
