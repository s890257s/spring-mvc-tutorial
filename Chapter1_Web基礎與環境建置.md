...
**私有網段不用死背，看久了自然有印象！**
以上三個私有網段是國際標準特別「保留」給私有網路使用的 IP 範圍。此外，172 網段的第二碼固定為 16 ~ 31，這是標準規範，並非講義寫錯。
...
> 💡 **知識預告**：透過 IP 地址與 NAT 機制，資料封包已經能成功送達正確的伺服器主機。但一台主機上可能同時運行著網頁伺服器與資料庫服務，系統該如何判斷資料要交接給哪一個軟體？這時就需要「Port 埠號」登場。

#### <a id="CH1-1-2-5"></a>[5. 網路服務的識別：通訊埠 Port](#toc)

一台電腦可以同時提供多種網路服務。如果說 IP 地址是整棟大樓的「地址」，那麼 Port 就像是大樓裡面的「分機號碼」或「各部門的收件窗口」，用來精準區分同一台電腦裡不同的網路服務。
...
| :----------------------------------------------- | :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **知名 / 系統通訊埠**<br>Well-Known Ports        | `0 ~ 1023`      | 保留給系統核心與常見網路基礎服務的專用 Port，如 HTTP (`80`)、FTP (`21`)、SSH (`22`)。<br>一般應用程式通常沒有權限也不建議使用此範圍。                       |
| **註冊通訊埠**<br>Registered Ports               | `1024 ~ 49151`  | 提供給一般軟體或企業應用程式使用的範圍。<br>例如資料庫 MySQL (`3306`)、伺服器 Tomcat (`8080`) 等。開發者撰寫自訂服務時，通常會選擇此區間的數字。            |
