...
- [3-2 阻絕資料外洩風險：Entity 與 DTO 的安全隔離](#CH3-2)
  - [1. 資料傳輸危機：Entity 的暴露風險與 DTO 防護機制](#CH3-2-1)
    - [1. 基本資料過濾：移除敏感資訊](#CH3-2-1-1)
    - [2. 多張表格資料合併：扁平化設計](#CH3-2-1-2)
    - [3. 資料預先格式化：符合 UI 視圖需求](#CH3-2-1-3)
  - [2. 轉換實作技巧：開發常見五大策略推演](#CH3-2-2)
    - [選項一：最簡單的映射方式：逐一屬性手動綁定](#CH3-2-2-1)
    - [選項二：封裝進 DTO 內部轉換 (高耦合，小專案限定)](#CH3-2-2-2)
    - [選項三：抽離為獨立的 MapperUtil](#CH3-2-2-3)
    - [選項四：使用 Spring BeanUtils 動態複製](#CH3-2-2-4)
    - [選項五：業界主流自動生成工具 MapStruct](#CH3-2-2-5)
...
#### <a id="CH3-2-1-1"></a>[1. 基本資料過濾：移除敏感資訊](#toc)
...
#### <a id="CH3-2-1-2"></a>[2. 多張表格資料合併：扁平化設計](#toc)
...
#### <a id="CH3-2-1-3"></a>[3. 資料預先格式化：符合 UI 視圖需求](#toc)
...
**📍 單元目標**  
認識並比較五種將 Entity 轉換為 DTO 的實作策略，最終掌握業界主流的 MapStruct 工具。

**📖 核心概念**  
將 Entity 轉換為 DTO 有多種做法，從最基礎的手動設定、封裝工具類，到自動化的 BeanUtils，各有優缺點。實務上為了兼顧效能與開發速度，常使用編譯期生成的自動映射工具。
...
#### <a id="CH3-2-2-1"></a>[選項一：最簡單的映射方式：逐一屬性手動綁定](#toc)
...
**📍 單元目標**  
了解為何需要從單純的 MVC 擴展至三層架構，並清楚定義各層的職責邊界與設計規範。

**🤔 為什麼需要它 / 只有 MVC 夠用嗎？**  
在早期的簡單系統中，Controller 除了接收請求，還常得兼顧「複雜的商業運算規則 (如折扣計算)」，甚至親自去「存取資料庫」。這會導致 Controller 膨脹成所謂的「上帝類別 God Class」，使得程式碼難以維護與重用。

為了解決這個問題，業界演進出了**三層架構** 規範，將系統解耦為「表現層、邏輯層及資料層」。**MVC 實際上只涵蓋了三層架構中的「Web 表現層與資料層」！**
...
**📍 單元目標**  
認識如何將上傳的檔案儲存到伺服器的實體硬碟中，並設定虛擬路由讓前端能夠順利讀取。
...
**📍 單元目標**  
建立登入機制，將使用者資訊存入 Session 中，並根據使用者的管理員身分 (`isAdmin`) 決定其操作權限。
...
**📍 單元目標**  
顯示所有會員清單；非管理員僅能查看清單，具備管理員權限者可執行新增 Create、修改 Update 與刪除 Delete。
...
**📍 單元目標**  
會員可以進入專屬的個人首頁，在上傳圖片後可以直接替換自己的大頭貼；此任務將運用上一節所學，將圖片轉化為 Byte Array 並存入資料庫。
